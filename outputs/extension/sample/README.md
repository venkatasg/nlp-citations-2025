# Sample run (extension experiment)
...
names produced by `src/analysis_api.py -e extension`. **Numbers in
this folder are not meaningful** — they are computed over a
six-paper subset of NAACL 2024 long papers.

To produce real numbers, run the full pipeline (see
`../../../README.md`):
...
takes ~10-12 h with an API key (1 req/s) and ~1 GB of disk
for `data/extension/raw/`.
